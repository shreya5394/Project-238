...
#User data in the form of Dictionaries
user_json = [
	{
		"name": "John Doe",
...
#Product data in the form of dictonaries
product_json = [
	{
...
#Function to drop and recreate the database tables
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()

#Function to add the data with the sample data
def seeder():
	#Adding the user data to the database 
	for user in user_json:
		Users.create(user.get("name"), user.get("email"), user.get("password"), user.get("contact"))

	#Adding products data the the product table
